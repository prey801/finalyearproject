...
class NIHMalariaDataset(Dataset):
    ...
    def __init__(self, split="train", image_size=224, transform=None):
        ...
        # Create class-to-index mapping from config.
        self.classes      = DatasetConfig.NIH_MALARIA_CLASSES
        self.class_to_idx = {cls_name: i for i, cls_name in enumerate(self.classes)}

        # FIX #5: Validate all labels before any training begins so unknown
        # labels raise a clear error instead of a mid-run KeyError from a worker.
        unknown = set(self.df["label"].unique()) - set(self.class_to_idx.keys())
        if unknown:
            raise ValueError(
                f"metadata.csv for split='{split}' contains unknown label(s): {unknown}. "
                f"Expected one of: {set(self.class_to_idx.keys())}"
            )
    ...
    def __getitem__(self, idx):
        ...
        # FIX #12: Access columns by name — not fragile positional index.
        ...
        # FIX #5 (continued): class_to_idx lookup is now guaranteed safe because
        # all labels were validated at __init__ time.
...
def get_dataloaders(image_size=224, batch_size=32, num_workers=4):
    ...
    # FIX #6: pin_memory is only beneficial (and safe) when CUDA is available.
    # Forcing it True on CPU-only machines wastes memory and triggers warnings.
